...
namespace Compiling
{
    public class IntersectConstructorParser : IPrefixParser
    {
        public IntersectConstructorParser(KeyParser parser)
        {
            Register(parser);
        }
        public Expression Parse(KeyParser parser, Token token, List<CompilingError> errors)
        {
            ...
            if (!parser.MatchValue(TokenValues.OpenBracket))
            {
                ...
                }
                {
                    ...
                    {
                        return new IntersectConstructorNode(token.Location,p1, p2);
                    }
                }
        ...
        {
            parser.RegisterPrefixParserByValue(TokenValues.Intersect, this);
        }
    }
}
